...
*/		
	...
	_addToBody: function (sBodyPart) {
		if (HtmlEditorField.htmlMode) {
			var sHtmlBody = HtmlEditorField.getHtml(true);
	...
	switchBodyToPlain: function ()
	{
		var
			...
			sHtmlSignature = (oIdentity !== null) ? oIdentity.getHtmlSignature() : '',
			sPlainSignature = (oIdentity !== null) ? oIdentity.getPlainSignature() : '',
			sHtmlBody = HtmlEditorField.getHtmlForSwitch(),
			iHtmlSignaturePos = sHtmlBody.lastIndexOf(sHtmlSignature),
			sHtmlBody1 = '',
			sHtmlBody2 = '',
			sNewPlainBody = ''
		;
		if (iHtmlSignaturePos > -1) {
			...
			sHtmlBody2 = sHtmlBody.slice(iHtmlSignaturePos + sHtmlSignature.length);
		}
		sNewPlainBody = TextFormatter.htmlToPlain(sHtmlBody1) + sPlainSignature + TextFormatter.htmlToPlain(sHtmlBody2);
		HtmlEditorField.setPlain(sNewPlainBody);
	},
