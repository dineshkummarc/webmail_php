...
CDataSource.prototype = {
	...
	_getXml: function(arParams, xml)
	{
		...
		if (window.CsrfToken !== undefined) {
			strResult += '<param name="token" value="' + window.CsrfToken + '"/>';
		}
	...
	_onParsingError: function (errorCode, action, request)
	{
	...
	parseXml: function(oXmlDoc, sAction, sRequest)
	{
		if (!oXmlDoc || !(oXmlDoc.documentElement) || typeof(oXmlDoc) !== 'object' || typeof(oXmlDoc.documentElement) !== 'object') {
			this._onParsingError(1, sAction, sRequest);
			return;
		}
		var oRootNode = oXmlDoc.documentElement;
		if (!oRootNode || oRootNode.tagName !== 'webmail') {
			this._onParsingError(2, sAction, sRequest);
			return;
		}
		...
		if (aNodes.length === 0 && sComplex !== 'messages_bodies') {
			if (!bBackground) {
				this._onParsingError(3, sAction, sRequest);
			}
			return;
		}
...
CCache.prototype = {
	...
	setMessageAppointment: function(msgId, msgUid, idFolder, folderFullName, oAppointment)
	{
		var dict = this._dictionaries[TYPE_MESSAGE];
		var keys = dict.keys();
		for (var i = 0; i < keys.length; i++) {
...
CHistoryStorage.prototype = {
	...
	_doStep: function (newKey) {
		if (Browser.mozilla && !WebMail.DataSource.waitMessagesBodies) {
			WebMail.DataSource.netLoader.abortRequests();
			WebMail.hideInfo();
			//this._inStep = false;
		}
		if (!Browser.opera && (!Browser.mozilla || !WebMail.DataSource.waitMessagesBodies)) {
