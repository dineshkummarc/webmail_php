...
var Dialog = {
	...
	alert: function (sText)
	{
		...
		this.open(sText, aButtons);
	},

	confirm: function (sText, fOk, fCancel, sOk)
	{
		...
		var aButtons = [
			{
				text: sOk,
				click: function() { fOk.call(); $(this).dialog('close'); }
			},
			{
				text: Lang.Cancel,
				click: function() { fCancel.call(); $(this).dialog('close'); }
			}
		];
		this.open(sText, aButtons);
	},

	open: function (sText, aButtons)
	{
		this._init();
		this._eText.innerHTML = sText;
		this._oDialog.dialog('option', 'buttons', aButtons);
		this._oDialog.dialog({ title: Lang.DialogAlertTitle });
