...
CWebMail.prototype = {
	...
	{
		this._oWaitMsgForNewWindow = null;
		var sDataKeys = this.DataSource.getStringDataKeyFromObj(TYPE_MESSAGE, oMsgHeaders);
		...
		var aParts = [PART_MESSAGE_HEADERS, PART_MESSAGE_HTML, PART_MESSAGE_ATTACHMENTS];
		var oListScreen = this.Screens[this.listScreenId];
		if (oListScreen && oListScreen.isDrafts()) {
			aParts = [PART_MESSAGE_HEADERS, PART_MESSAGE_HTML, PART_MESSAGE_UNMODIFIED_PLAIN_TEXT, PART_MESSAGE_ATTACHMENTS];
		}
	...
	_onMessageSaving: function (oSaveMsg)
	{
		...
		ClearDraftsAndSetMessageId(oSaveMsg.id, oSaveMsg.uid);
...
//						break;
			...
			case TYPE_MESSAGE_LIST:
				...
				if (this._oWaitMsgListForLastMsg !== null && this._oWaitMsgListForFirstMsg.isEqualMsgList(data)) {
	...
	{
		var screen = this.Screens[this.listScreenId];
		if (screen) {screen.startCheckMail(true);}
	},
	...
	SetActiveTab: function ()
	{
		...
		if (!this.Settings.allowCalendar || window.Seporated || !window.UseDb) {
	...
	blur: function (strAddedTitle)
	{
		...
		var newMsgsCount = listScreen.GetNewMsgsCount();
		if (newMsgsCount > 0) {
			var blurTitle = Lang.TitleNewMessagesCount.replace(/%count%/, newMsgsCount);
			this._setTitle(blurTitle, true);
			if (!Browser.ie) {
				Favicon.animate(['favicon-blink.ico', 'favicon.ico'], 1000);
			}
		}
		else {
			this._setTitle(strAddedTitle);
		}
	},
	...
	focus: function ()
	{
		...
		if (Browser.chrome) {
			if (this._chromeFocusIframe === undefined) {
				this._chromeFocusIframe = CreateChild(document.body, 'iframe',
					[['src', 'about:blank'], ['class', 'wm_hide']]);
			}
			this._chromeFocusIframe.src += '';
		}
	},

	setTitle: function (sTitleToAdd, addedTitleFirst, discardScreenTitle)
	{
		this._setTitle(sTitleToAdd, addedTitleFirst, discardScreenTitle);
	},

	_setTitle: function (sTitleToAdd, addedTitleFirst, discardScreenTitle)
	{
		var sBaseTitle = this._title;
		if (Screens[this.ScreenId] != undefined && (!discardScreenTitle)) {
			var titleLangField = Screens[this.ScreenId].titleLangField;
			sBaseTitle += ' - ' + Lang[titleLangField];
		}
		if (typeof(sTitleToAdd) == 'string' && sTitleToAdd.length > 0) {
			if (addedTitleFirst) {
				document.title = sTitleToAdd + ' - ' + sBaseTitle;
			}
			else {
				document.title = sBaseTitle + ' - ' + sTitleToAdd;
			}
		}
		else {
			document.title = sBaseTitle;
		}
	},
	...
	buildAccountsList: function()
	{
		...
		this._calendarTab.className = (!this.Settings.allowCalendar || window.Seporated || !window.UseDb)
...
CSettingsList.prototype = {
	...
	getFromXml: function(rootElement)
	{
		...
		this.allowContacts = XmlHelper.getBoolAttributeByName(rootElement, 'allow_contacts', this.allowContacts);
