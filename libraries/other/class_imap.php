		...
		if ($response)
		{
			...
			if (is_array($aPreResult) && 0 < count($aPreResult))
			{
				...
				foreach ($aPreResult as $iIndex => $aPart)
				{
					if (isset($aPart[1]))
					{
						$aNewResult[$iIndex] = ConvertUtils::GetTimeFromString($aPart[1]);
					}
				}
...
Example:    search_mailbox("FLAGGED SINCE 1-Feb-1994 NOT FROM \"Smith\"")
	...
	*/
	 ...
	 * @return array|false
	 */
	function _in_search_mailbox($search_cri, $charset = '', $_isUidSearch = false, $order_by = null)
	{
		if (!$this->_checkState_0('SELECTED')) return false;
		...
		if ($this->put_line($cmd))
		{
			...
			if (!$this->_checkResponse($response))
			{
				...
				// try without charset
				else if ($this->put_line($cmd_wo_charset))
				{
					$response = $this->get_server_responce();
					if (!$this->_checkResponse($response))
					{
	 ...
	 * @return array|false
	 */
	function search_mailbox($search_cri, $charset = '', $order_by = null)
	{
		return $this->_in_search_mailbox($search_cri, $charset, false, $order_by);
	}
	 ...
	 * @return array|false
	 */
	function uid_search_mailbox($search_cri, $charset = '', $order_by = null)
	{
