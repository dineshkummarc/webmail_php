...
class EnumConvert
{
	...
	static function Validate($mValue, $sClassName)
	{
		...
		$sResult = null;
		foreach ($aConsts as $iEnumValue)
		{
			if ($mValue === $iEnumValue)
			{
				$sResult = $mValue;
				break;
			}
	 ...
	 * @return string
	 */
	public static function ToXml($iValue, $sClassName)
	{
		$aConsts = EnumConvert::GetInst($sClassName);

		$sResult = '';
		$iValue = (int) $iValue;
		foreach ($aConsts as $sKey => $iEnumValue)
		{
			if ($iValue === $iEnumValue)
			{
				$sResult = $sKey;
				break;
			}
	 ...
	 * @return string
	 */
	public static function ToPost($iValue, $sClassName)
	{
		return self::ToXml($iValue, $sClassName);
	}
}
