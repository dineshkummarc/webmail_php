...
FROM %sawm_addr_book WHERE id_user = %d AND deleted = 0 AND auto_create = 0 LIMIT %d, %d';
	...
	public function GetSuggestContactItems($iUserId, $sSearch, $iRequestLimit)
	{
		...
		if (!empty($sSearch))
		{
			...
			$sSearchAdd = sprintf(' AND (fullname LIKE %s OR h_email LIKE %s OR b_email LIKE %s OR other_email LIKE %s)',
				$sSearch, $sSearch, $sSearch, $sSearch);
		}
...
INNER JOIN %sawm_addr_groups_contacts AS gr_cnt ON gr_cnt.id_addr = book.id_addr AND gr_cnt.id_group = %d',
		...
		{
			$sSearch = $this->escapeString('%'.$sSearch.'%');
			$sSearchAdd .= sprintf(' AND (book.fullname LIKE %s OR book.h_email LIKE %s OR book.b_email LIKE %s OR book.other_email LIKE %s)',
				$sSearch, $sSearch, $sSearch, $sSearch);
		}
...
book.use_frequency, book.fullname, book.use_friendly_nm
