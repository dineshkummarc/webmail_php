...
class CAppXmlBuilder
{
	 ...
	 * @param int $iNewContactId = -1
	 * @return void
	 */
	public static function BuildContactList(CXmlDocument &$oRequestXml, CXmlDocument &$oResultXml, $oAccount, $oApiContactsManager, $iNewContactId = -1)
	{
		...
		$oContactsNode->AppendAttribute('id_group', (int) $sGroupId);
		$oContactsNode->AppendAttribute('added_contact_id', (int) $iNewContactId);
		...
		if ($iNewContactId > 0)
		{
			$oNewContact = $oApiContactsManager->GetContactById($oAccount->IdUser, $iNewContactId);
			if ($oNewContact)
			{
	 ...
	 * @return void
	 */
	public static function BuildContact(CXmlDocument &$oResultXml, $oContact, $oApiContactsManager = null)
	{
		...
		$oContactNode->AppendChild(new CXmlDomNode('surname', $oContact->SurName, true));
		...
		&$oMailProcessor, $oMessage, $oFolder, $iMode, $iCharsetNum, $bShowImages)
	{
		...
		if (($iMode & 8) == 8)
		{
			if (!$bShowImages)
			{
				$oMessageNode->AppendChild(new CXmlDomNode('reply_html',
					ConvertUtils::AddToLinkMailToCheck(
						ConvertUtils::HtmlBodyWithoutImages(
							ConvertUtils::ReplaceJSMethod(
								$oMessage->GetRelpyAsHtml(true, $iAccountOffset, $oMessageInfo)))), true, true));
		...
		if (($iMode & 16) == 16)
		{
			$oMessageNode->AppendChild(new CXmlDomNode('reply_plain',
				ConvertUtils::AddToLinkMailToCheck(
					$oMessage->GetRelpyAsPlain(true, $iAccountOffset)), true, true));
		}
		...
		if (($iMode & 32) == 32)
		{
			if (!$bShowImages)
			{
				$oMessageNode->AppendChild(new CXmlDomNode('forward_html',
					ConvertUtils::AddToLinkMailToCheck(
						ConvertUtils::HtmlBodyWithoutImages(ConvertUtils::ReplaceJSMethod($oMessage->GetRelpyAsHtml(true, $iAccountOffset, $oMessageInfo)))), true, true));
			...
			else
			{
				$oMessageNode->AppendChild(new CXmlDomNode('forward_html',
					ConvertUtils::AddToLinkMailToCheck(
						ConvertUtils::ReplaceJSMethod($oMessage->GetRelpyAsHtml(true, $iAccountOffset, $oMessageInfo))), true, true));
			}
		}

		if (($iMode & 64) == 64)
		{
			$oMessageNode->AppendChild(new CXmlDomNode('forward_plain',
				ConvertUtils::AddToLinkMailToCheck($oMessage->GetRelpyAsPlain(true, $iAccountOffset)), true, true));
		}
		...
		if (($iMode & 256) == 256 || ($iMode & 8) == 8 || ($iMode & 16) == 16 || ($iMode & 32) == 32 || ($iMode & 64) == 64)
		{
			...
			if ($oAttachments && 0 < $oAttachments->Count())
			{
				...
				foreach ($aAttachmentsKeys as $iAttachKey)
				{
					...
					//++ICal
					if (false && $bParseICal && empty($sICal) && $oAttachment)
					{
		...
		//++ICal
		if (false && $bParseICal && !empty($sICal))
		{
			$mResult = false;
			$oApiCalendarManager = CApi::Manager('calendar');
			if ($oApiCalendarManager)
			{
				$mResult = $oApiCalendarManager->IcsPreprocessAppointment($oAccount, trim($sICal));
				...
				if (is_array($mResult) && !empty($mResult['Action']) && !empty($mResult['Body']))
				{
					...
					{
						$oAppointmentNode = new CXmlDomNode('appointment');
						$oAppointmentNode->AppendAttribute('file', $sTemptFile);
						$oAppointmentNode->AppendAttribute('type', $mResult['Action']);
						if (!empty($mResult['Location']))
						{
							$oAppointmentNode->AppendChild(new CXmlDomNode('location', $mResult['Location'], true));
						}
						if (!empty($mResult['Description']))
						{
							$oAppointmentNode->AppendChild(new CXmlDomNode('description', $mResult['Description'], true));
						}
						if (!empty($mResult['When']))
						{
							$oAppointmentNode->AppendChild(new CXmlDomNode('when', $mResult['When'], true));
						}
						if (!empty($mResult['CalendarId']))
						{
							$oAppointmentNode->AppendChild(new CXmlDomNode('calendar_id', $mResult['CalendarId'], true));
						}
						...
						if (isset($mResult['Calendars']) && is_array($mResult['Calendars']) && 0 < count($mResult['Calendars']))
						{
							...
							$oAppointmentNode->AppendChild($oCalendarsNode);
						}

						$oMessageNode->AppendChild($oAppointmentNode);
					}
					else
					{
		...
		if (($iMode & 512) == 512)
		{
			$oMessageNode->AppendChild(new CXmlDomNode('unmodified_plain_text', $oMessage->GetNotCensoredTextBody(true), true, true));
		}
	...
	public static function BuildSettingsList(CXmlDocument &$oResultXml, CAccount $oAccount, $oDomain)
	{
		...
		$oSettingsListNode->AppendAttribute('allow_contacts', (int) $oAccount->User->AllowContacts);
		...
		$oSettingsListNode->AppendAttribute('allow_calendar', (int) 
			($oApiCapabilityManager->IsCalendarSupported() && $oAccount->User->AllowCalendar));
		...
		$oSettingsListNode->AppendAttribute('mobile_sync_enable_system', (int) 
			(!empty($sUrl) && (bool) $oSettings->GetConf('Common/EnableMobileSync')));
		...
		$oSettingsListNode->AppendAttribute('show_global_contacts', (int) 
		...
		if ($oSettings->GetConf('WebMail/EnableLastLoginNotification'))
		{
			...
			if (!empty($sLastLogin))
			{
				$oLastLoginDate = new CDateTime((int) $sLastLogin + 2 * $oAccount->GetDefaultTimeOffset() * 60);
