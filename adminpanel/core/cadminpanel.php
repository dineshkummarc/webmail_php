	...
	protected function initScreen($sTab)
	{
		...
		if ($this->IsAuth())
		{
			$aScreenMap = array(
				...
				AP_TAB_SUBADMINS => 'ap_Table_Screen',
...
<div class="wm_tabslist_item_small"><a href="'.AP_INDEX_FILE.'?help" target="_blank">Help</a></div></div>';
	...
	protected function initAdminPanel()
	{
		...
		try
		{
			...
			if ($bResetToDefault)
			{
				$this->sTab = $this->IsSubAdminAuthType() ? AP_TAB_SUB_DEFAULT : AP_TAB_DEFAULT;
	...
	protected function initAuth()
	{
		...
		if (isset($_GET['login']) && CPost::Has('AdmloginInput') && CPost::Has('AdmpasswordInput'))
		{
			if ($this->CallModuleFunction('CCommonModule', 'AuthLogin',
				array(CPost::Get('AdmloginInput'), CPost::Get('AdmpasswordInput'))))
			{
	...
	public function HasAccessDomain($iDomainId)
	{
		...
		$bResult = false;
		if (AP_SESS_AUTH_TYPE_SUBADMIN === $this->iAuthType)
		{
			if (is_array($this->aAuthDomains))
			{
				$bResult = in_array($iDomainId, $this->aAuthDomains);
			}
		}
		else if (AP_SESS_AUTH_TYPE_SUPER_ADMIN_ONLYREAD === $this->iAuthType)
		{
	 ...
	 * @return bool
	 */
	public function IsSubAdminAuthType()
	{
		return AP_SESS_AUTH_TYPE_SUBADMIN === $this->iAuthType;
	}
