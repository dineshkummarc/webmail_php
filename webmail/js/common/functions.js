...
function HtmlEncode(source, encodeQuotes)
{
	source = source || '';
...
{
	if (!encoded) {
		source = (isBody) ? HtmlEncodeBody(source) : HtmlEncode(source);
	}
	return '<![CDATA[' + source + ']]>';
}
...
function FlashVersion() {
	...
	if (Browser.ie) {
		...
		for (i = 0; i < navigator.plugins.length; i++) {
			...
			if (oPlugin.name.indexOf('Flash') > -1) {
				var iParsedVersion = parseInt(oPlugin.description.substring(16));
