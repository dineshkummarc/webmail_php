...
CAccounts.prototype = {
	...
	{
		this.editableId = XmlHelper.getIntAttributeByName(rootElement, 'last_id', this.editableId);
...
CMessage.prototype = {
	...
	_getArrayRecipients: function (sRecipients)
	{
		...
		var aRecipients = Array();
		var sWorkingRecipients = Trim(HtmlDecode(sRecipients)) + " ";
				...
				default:
					...
					if (!isInAngleBrackets && !isInBrackets && !isInQuotes) {
						...
						if (Trim(str).length > 0) {
							var oRecipient = GetEmailParts(str);
							var inList = false;
							var jCount = aRecipients.length;
	...
	_readAttachment: function (attachNode)
	{
		...
		if (sVoice != '#') {
			...
			return { sDownload: sDownload, sVoice: sVoice, iDuration: iDuration,
	...
	_readAppointment: function (eRoot)
	{
		var
			eAppointment = XmlHelper.getFirstChildNodeByName(eRoot, 'appointment'),
			sFile, sAppType, sType, aType,
			sConfig = '',
			...
			aTypes = [EnumAppointmentType.Request, EnumAppointmentType.Reply, EnumAppointmentType.Cancel],
			aCalendars = [],
			eCalId = null,
			sCalId = '',
			eLocation = null,
		...
		if (eAppointment !== null) {
			...
			sAppType = XmlHelper.getAttributeByName(eAppointment, 'type', '');
			aType = sAppType.split('-');
			sType = aType.shift();
			if (-1 === $.inArray(sType, aTypes)) {
				WebMail.showError('Unknown appointment type: ' + sAppType);
			}
			sConfig = aType.join('-');
			...
			sDescription = XmlHelper.getFirstChildValue(eDescription, '');
			
			eWhen = XmlHelper.getFirstChildNodeByName(eAppointment, 'when');
			sWhen = XmlHelper.getFirstChildValue(eWhen, '');
			
			this.oAppointment = { sFile: sFile, sType: sType, sConfig: sConfig, aCalendars: aCalendars,
	...
	getFromXml: function(rootElement)
	{
		...
		this.mailConfirmationValue = XmlHelper.getFirstChildValue(mailConfirmationNode, this.mailConfirmationValue);
		...
		this._readAppointment(rootElement);
