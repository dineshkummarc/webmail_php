...
CContactsScreen.prototype = {
	placeData: function(data)
	{
		switch (data.type){
			...
			case TYPE_GLOBAL_CONTACTS:
				if (this.bGlobal && data.type === TYPE_CONTACTS
					|| !this.bGlobal && data.type === TYPE_GLOBAL_CONTACTS) {
					break;
				}
    ...
    _requestContacts: function (oRequestArgs)
    {
        var
			oContactsGroups = new CContacts(oRequestArgs.sGroupId, oRequestArgs.lookFor,
				oRequestArgs.lookFirstChar),
			sXml = oContactsGroups.getInXml()
		;
		GetHandler(this.iContactsType, oRequestArgs, [], sXml);
    },
		...
		switch (historyArgs.Entity) {
			...
			case PART_EDIT_CONTACT:
				if (this.Contact.sContactId === historyArgs.sContactId) {
					...
					GetHandler(this.iContactType, { sContactId: historyArgs.sContactId }, [], '');
				}
			break;
		...
		if (this.oSelection != null) {
			...
			if (iCount > 0) {
				...
				if (contacts.length != 0 || groups.length != 0) {
					...
					var sXml = this.getXmlParams();
					sXml += '<contacts>' + contacts + '</contacts>';
					sXml += '<groups>' + groups + '</groups>';
					Dialog.confirm(Lang.ConfirmAreYouSure, (function (sXml) {
						return function () {
							WebMail.DataSource.cache.clearAllContactsGroupsList();
	...
	_buildToolBar: function(PopupMenus)
	{
		...
		this.eAddContactsTool = toolBar.addMoveItem(TOOLBAR_ADD_CONTACTS_TO, PopupMenus, this._contactsToMenu, true);
	...
	applyGlobalPersonal: function () {
		...
		if (this.bGlobal) {
			...
			this.eAddContactsTool.className = 'wm_hide';
			...
			this.eAddContactsTool.className = 'wm_toolbar_item';
