...
// LOGIN
		...
		if ($oAccount)
		{
			...
			if (!empty($sTempFile) && !empty($sAction) && !empty($sCalendarId))
			{
				...
				if (!empty($sData))
				{
					...
					$bResult = $oCalendarApi->IcsProcessAppointment($oAccount, $sAction, $sCalendarId, $sData);
					if (true === $bResult)
					{
						$this->SetUpdateResponse('process_appointment');
					}
					else
					{
						$this->SetErrorResponse(WebMailException); // TODO
	...
	public function DoResetpassword()
	{
		...
		if (1 === $iStep)
		{
			...
			{
				if (!empty($oAccount->User->Question1) && !empty($oAccount->User->Question2))
				{
	...
	public function DoRegistration()
	{
		...
		{
			$sCaptchaError = '';
			$sCaptcha = $this->oRequestXml->GetParamValueByName('captcha');
			if (!CSession::Has('captcha_keystring') || $sCaptcha !== CSession::Get('captcha_keystring', null))
			{
				$sCaptchaError = CaptchaError;
			}
	...
	public function DoLogin()
	{
		...
		{
			CSession::Set(API_JS_TIMEOFFSET, $sJsTimeOffset);
		}
...
//			}
	...
	public function DoGetGlobalContact()
	{
		...
		if ($oAccount)
		{
				...
				$oApiCollaborationManager->IsContactsGlobalSupported())
			{
				...
				if ($oContact)
				{
					CAppXmlBuilder::BuildContact($this->oResultXml, $oContact);
				}
				else
				{
					$this->SetErrorResponse(PROC_CANT_GET_CONT_FROM_DB);
	...
	public function DoGetMobileSync()
	{
		...
		if ($oAccount)
		{
			...
			if ($bEnableMobileSync && $oDefAccount)
			{
				$oMobileSyncNode->AppendChild(new CXmlDomNode('login', 
					$oApiDavManager->GetLogin($oDefAccount), true));
					
				$oMobileSyncNode->AppendChild(new CXmlDomNode('dav_url',
					$oApiDavManager->GetServerUrl(), true));
				
				$oMobileSyncNode->AppendChild(new CXmlDomNode('principal_url',
					$oApiDavManager->GetPrincipalUrl($oDefAccount), true));
			}
			else
			{
				$oMobileSyncNode->AppendChild(new CXmlDomNode('login', '', true));
				$oMobileSyncNode->AppendChild(new CXmlDomNode('dav_url', '', true));
...
// OTHER
	...
	{
		$sToken = (string) CApi::GetCsrfToken(false);
		return false === CSession::Get(APP_SESSION_ACCOUNT_ID, false) ||
			!(!empty($sToken) && (string) $this->oRequestXml->GetParamValueByName('token') !== $sToken);
	}
