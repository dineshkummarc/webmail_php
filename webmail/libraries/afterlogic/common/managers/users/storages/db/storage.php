				...
				else
				{
					...
					if ($bIsDefaultAccount)
					{
						...
						if (is_array($aAccountsId) && 0 < count($aAccountsId))
						{
							...
							$bResult &= $this->oConnection->Execute($this->oCommandCreator->DeleteGroupsContacts($iUserId));
							$bResult &= $this->oConnection->Execute($this->oCommandCreator->DeleteContacts($iUserId));
							$bResult &= $this->oConnection->Execute($this->oCommandCreator->DeleteGroups($iUserId));
							...
							// Funumbol (without result validation)
//							$this->oConnection->Execute($this->oCommandCreator->DeleteFnblPimContact($sEmail));
//							$this->oConnection->Execute($this->oCommandCreator->DeleteFnblPimCalendar($sEmail));
//							$this->oConnection->SetException(null);
						}
					}
					else
					{
	 ...
	 * @return array | false [IdAccount => [IsMailingList, Email, FriendlyName, IsDisabled, IdUser]]
	 */
				...
				$this->dbOrderBy($sOrderBy), $bOrderType, $sSearchDesc)))
		{
			...
			while (false !== ($oRow = $this->oConnection->GetNextRecord()))
			{
				$aUsers[$oRow->id_acct] = array(
					(bool) $oRow->mailing_list, $oRow->email, $oRow->friendly_nm, (bool) $oRow->deleted, $oRow->id_user);
			}
		}
		$this->throwDbExceptionIfExist();
		return $aUsers;
