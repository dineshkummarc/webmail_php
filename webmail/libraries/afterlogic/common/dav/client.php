...
class DAVClient 
{
	...
	*/
	public function __construct($baseUrl, $user, $pass)
 	{
		$this->client = new afterlogic_DAV_Client(
				array(
					'baseUri' => $baseUrl,
					'userName' => $user,
					'password' => $pass
	...
	*/	
	function GetUUID()
	{
		return Sabre_DAV_UUIDUtil::getUUID();
	}	
	...
	*/	
	...
	{
		list($user, $domain) = explode('@', $this->user);
		$newUser = $username.'-'.str_replace('-' , '', $this->GetUUID()).'@'.$domain;

		if($domain == '') $newUser = rtrim($newUser, '@');
	...
	* @return string
	*/
	function CreateEvent($url, $icalData) 
	{
	...
	* @return string
	*/
	function UpdateEvent($url, $icalData) 
	{
		return $this->client->request(
				'PUT', 
				$url, 
...
<c:mkcalendar xmlns:c="'.Sabre_CalDAV_Plugin::NS_CALDAV.'" xmlns:d="DAV:" xmlns:ic="http://apple.com/ns/ical/">
	<d:set>
		<d:prop>
	...
	* @return array
	*/
	function QueryCal($filter, $url = '', $recurrenceSet = '') 
	{
		$xml = 
'<?xml version="1.0" encoding="utf-8" ?>
<c:calendar-query xmlns:d="DAV:" xmlns:c="'.Sabre_CalDAV_Plugin::NS_CALDAV.'">
	<d:prop>
		<d:getetag/>
        <c:calendar-data>
			'.$recurrenceSet.'
	...
	*/
	function QueryCard($filter, $url = '') 
	{
		$xml = 
'<?xml version="1.0" encoding="utf-8" ?>
<c:addressbook-query xmlns:d="DAV:" xmlns:c="'.Sabre_CardDAV_Plugin::NS_CARDDAV.'">
	<d:prop>
		<d:getetag/>
		<c:address-data>
			<c:allprop />
	...
	'.$filter.'
		...
		foreach($aStatus as $key => $props) 
		{
			$response = array();
			$response['href'] = basename($key);
			$response['ctag'] = isset($props[200]) ? preg_replace('/^"?([^"]+)"?/', '$1', $props[200][self::PROP_CTAG]) : '';
			$response['data'] = isset($props[200]) ? $props[200][self::PROP_CALENDAR_DATA] : '';
	...
	function GetVcards($url = '', $start = null, $finish = null) 
	{    
...
//		$recurrenceSet = '<c:limit-recurrence-set start="'.$start.'" end="'.$finish.'"/>';
		...
		return $this->QueryCal($filter, $url/*, $recurrenceSet*/);
	}
...
'<c:filter>
	...
	*/
	...
	function GetCalendars($url = '')
	{
		...
		{
			if ($props['{DAV:}resourcetype']->is('{'.Sabre_CalDAV_Plugin::NS_CALDAV.'}calendar')) 
			{
				$calendar = new CalendarInfo($key);
...
'<c:filter>
	...
	function AddProxy($proxy, $to)
	{
		...
		$sProxyStr .= '<d:href>' . "principals/" . $to . '</d:href>';
	...
	function DeleteProxy($proxy, $to)
	{
		...
		$sProxyStr = str_replace('<d:href>' . 'principals/' . $to . '</d:href>', '', $sProxyStr);
...
<d:expand-property xmlns:d="DAV:">
    <d:property name="calendar-proxy-read-for" namespace="http://calendarserver.org/ns/">
        <d:property name="calendar-home-set" namespace="'.Sabre_CalDAV_Plugin::NS_CALDAV.'" />
    </d:property>
    <d:property name="calendar-proxy-write-for" namespace="http://calendarserver.org/ns/">
		...
		if(isset($res['body']))
		{
			...
			$xp->registerNamespace('c', Sabre_CalDAV_Plugin::NS_CALDAV);
			$xp->registerNamespace('cs', Sabre_CalDAV_Plugin::NS_CALENDARSERVER);
